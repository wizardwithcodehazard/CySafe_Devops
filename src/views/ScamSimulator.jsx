...
export default function ScamSimulator({ isDark }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b141a] relative pb-20">
        ...
        {!scenario ? (
          ...
          <>
            ...
            {messages.map((m, i) => (
              ...
              <div className={`max-w-[85%] p-2.5 rounded-lg text-sm shadow-md relative leading-relaxed ${
                m.isUser ? 'bg-[#005c4b] text-white rounded-tr-none' : 'bg-[#202c33] text-white rounded-tl-none'
              }`}>
                 {m.type === 'image' && (
      ...
      {/* FIXED UI: High Contrast Choices */}
